...
export function LiveTrackingPanel({ ticket, isProvider }: Readonly<Props>) {
  ...
  useEffect(() => {
    ...
    if (!("geolocation" in navigator)) {
      setGeoError(t("tracking.errorNoGeolocation"));
      setSharing(false);
      return;
    }
    setGeoError(null);
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      ...
      (err) => {
        if (err.code === err.PERMISSION_DENIED) {
          setGeoError(t("tracking.errorPermissionDenied"));
        } else if (err.code === err.POSITION_UNAVAILABLE) {
          setGeoError(t("tracking.errorPositionUnavailable"));
        } else {
          setGeoError(t("tracking.errorLocationFailed"));
        }
        setSharing(false);
      },
      ...
      <div style={{ padding: "16px 18px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 15, fontWeight: 700, marginTop: 2 }}>
            {isProvider ? t("tracking.providerHeadline") : t("tracking.customerHeadline")}
          </div>
        </div>
        <span style={{ flex: 1 }} />
        <span
        ...
        >
          ...
          {connected ? t("tracking.statusLive") : t("tracking.statusConnecting")}
        </span>
      </div>
        ...
        {hasDest ? (
          ...
          <div style={{ height: "100%", display: "grid", placeItems: "center", color: "var(--text-muted)", fontSize: 13, background: "var(--slate-50, #f8fafc)", textAlign: "center", padding: "0 24px" }}>
            {seedQuery.isFetching
              ? t("tracking.loadingDestination")
              : t("tracking.errorNoAddress")}
          </div>
        )}
      </div>
      ...
      <div style={{ padding: "14px 18px", display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          >
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 600, marginTop: 2, wordBreak: "break-word" }}>
                {snapshot!.destAddress || ticket.location || t("tracking.customerLocation")}
              </div>
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              >
                ...
                {t("tracking.appleMaps")}
              </a>
            </div>
            {!hasProvider && (
              <span style={{ fontSize: 11.5, color: "var(--text-muted)" }}>
                {t("tracking.tipShareFirst")}
              </span>
            )}
          </div>
        )}
        ...
        {hasProvider ? (
          ...
          <div style={{ fontSize: 13, color: "var(--text-muted)" }}>
            {isProvider
              ? (sharing
                  ? t("tracking.gettingLocation")
                  : t("tracking.sharingOff"))
        ...
        {isProvider && (
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            >
              {sharing ? t("tracking.stopSharing") : t("tracking.shareLocation")}
            </button>
            {sharing && (
              <span style={{ fontSize: 12.5, color: "var(--emerald-700)" }}>
                {t("tracking.broadcasting")}
              </span>
            )}
          </div>
        )}
        ...
        {isProvider && sharing && accuracyMeters != null && (
          <span style={{ fontSize: 11.5, color: "var(--text-muted)" }}>
            {t("tracking.accuracy", { meters: Math.round(accuracyMeters) })}
            {accuracyMeters > 200 && ` ${t("tracking.accuracyWifi")}`}
          </span>
        )}
