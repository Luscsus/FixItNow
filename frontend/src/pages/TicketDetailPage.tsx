  ...
  return (
    <>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(130px, 1fr))", gap: 10 }}>
        {urls.map((url, i) => (
          <button key={i} onClick={() => setLightboxIdx(i)} style={{ border: "1px solid var(--border)", padding: 0, borderRadius: 10, overflow: "hidden", aspectRatio: "1", cursor: "zoom-in", background: "var(--slate-100)", transition: "opacity 0.15s" }}
      ...
      {lightboxIdx !== null && (
        <div role="dialog" aria-modal="true" aria-label="Image viewer" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.9)", zIndex: 1000, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }} onClick={() => setLightboxIdx(null)}>
          <img src={urls[lightboxIdx]} alt={`Photo ${lightboxIdx + 1}`} onClick={(e) => e.stopPropagation()} style={{ maxWidth: "min(90vw, 1100px)", maxHeight: "80vh", objectFit: "contain", borderRadius: 10, boxShadow: "0 24px 64px rgba(0,0,0,0.6)", userSelect: "none" }} />
          <div style={{ marginTop: 14, color: "rgba(255,255,255,0.5)", fontSize: 12, fontFamily: "var(--font-mono)", letterSpacing: "0.08em" }}>{lightboxIdx + 1} / {urls.length}</div>
          {lightboxIdx > 0 && <button style={{ ...navBtn, left: 20 }} onClick={(e) => { e.stopPropagation(); setLightboxIdx(lightboxIdx - 1); }} aria-label="Previous image">‹</button>}
          {lightboxIdx < urls.length - 1 && <button style={{ ...navBtn, right: 20 }} onClick={(e) => { e.stopPropagation(); setLightboxIdx(lightboxIdx + 1); }} aria-label="Next image">›</button>}
          <button onClick={() => setLightboxIdx(null)} aria-label="Close" style={{ position: "absolute", top: 18, right: 22, background: "rgba(255,255,255,0.1)", border: "none", color: "#fff", borderRadius: 8, width: 36, height: 36, fontSize: 18, cursor: "pointer", display: "grid", placeItems: "center", backdropFilter: "blur(4px)" }}>✕</button>
        </div>
      )}
    </>
  );
        ...
        <div>
          ...
          <div className="row" style={{ marginTop: 28, gap: 12, flexWrap: "wrap" }}>
            ...
            {isProvider && ticket.status === "PENDING_APPROVAL" && (() => {
              ...
              const handleAccept = () => {
                if (isUnassigned) { acceptMut.mutate(ticket.id, { onSuccess: refreshTicket }); }
                else if (ticket.requestedStartAt) { confirmMut.mutate(ticket.id, { onSuccess: refreshTicket }); }
                else { updateMut.mutate({ ticketId: ticket.id, status: "APPROVED" }); }
              };
