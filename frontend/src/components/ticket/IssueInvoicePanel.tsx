...
export function IssueInvoicePanel({ ticket }: Props) {
  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: '24px 24px 20px', boxShadow: 'var(--shadow-sm)' }}>
      <div style={{ marginBottom: 16 }}>
        <div className="mono" style={{ fontSize: 10.5, color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 6 }}>
          {t('invoicePanel.issueInvoice')}
        </div>
        <p style={{ margin: 0, fontSize: 14, color: 'var(--text-muted)', lineHeight: 1.5 }}>
          {t('invoicePanel.enterAmount', { name: ticket.submittedByName ?? 'the customer' })}
        </p>
        {provider && !bankReady && (
          <div role="alert" style={{ marginTop: 12, padding: '10px 12px', borderRadius: 8, background: 'var(--amber-50, #fffbeb)', border: '1px solid var(--amber-300, #fcd34d)', color: 'var(--amber-900, #78350f)', fontSize: 13, lineHeight: 1.45 }}>
            {t('invoicePanel.payoutMissing')}{' '}
            <Link to="/profile/edit" style={{ color: 'inherit', textDecoration: 'underline', fontWeight: 600 }}>
              {t('invoicePanel.addNow')}
            </Link>
          </div>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
          <div style={{ position: 'relative', flex: 1 }}>
            ...
            <input
              ...
              style={{ width: '100%', boxSizing: 'border-box', padding: '10px 12px 10px 26px', fontSize: 16, fontWeight: 600, border: '1px solid var(--border)', borderRadius: 10, background: 'var(--slate-50, #f8fafc)', color: 'var(--text)', outline: 'none' }}
            />
          </div>
          <button type="button" className="btn btn-secondary btn-sm" onClick={handlePreview} disabled={mutation.isPending} title={t('invoicePanel.preview')}>
            {t('invoicePanel.preview')}
          </button>
        </div>
        ...
        {mutation.isSuccess && (
          <div style={{ fontSize: 12, color: '#166534', background: '#DCFCE7', padding: '6px 10px', borderRadius: 6, marginBottom: 10 }}>
            {t('invoicePanel.success')}
          </div>
        )}

        <button type="submit" className="btn btn-primary" disabled={mutation.isPending || !amount || !bankReady} style={{ width: '100%' }}>
          {mutation.isPending ? t('invoicePanel.sending') : t('invoicePanel.sendInvoice')}
        </button>
      </form>
    </div>
  );
