...
type Tab = "Overview" | "Schedule" | "Jobs" | "Reviews";

const ACTIVE_STATUSES = new Set(["APPROVED", "IN_TRANSIT", "PENDING_PROVIDER_INVOICE", "PENDING_PAYMENT"]);
...
export function ProviderAccountPage() {
  ...
  const { data: fetchedProvider } = useProviderByIdQuery(urlId && !stateProvider ? urlId : "");
  ...
  const completedTickets = providerTickets.filter((t2) => t2.status === "COMPLETED");
  const activeTickets    = providerTickets.filter((t2) => ACTIVE_STATUSES.has(t2.status));
  const inboundTickets   = providerTickets.filter((t2) => t2.status === "PENDING_APPROVAL");
  const totalEarned = completedTickets.reduce((sum, t2) => sum + (t2.estimatedCost ?? 0), 0);
  const stats = {
    completedJobs:   completedTickets.length,
    activeJobs:      activeTickets.length,
    inboundRequests: inboundTickets.length,
    totalEarned:     completedTickets.some((t2) => t2.estimatedCost != null) ? totalEarned : null,
  };

  const bankInfoMissing =
    !!ownProfile &&
    (!ownProfile.bankAccountHolder?.trim() || !ownProfile.bankIban?.trim()
      || !ownProfile.bankBic?.trim() || !ownProfile.bankName?.trim());

  const TABS: { key: Tab; label: string }[] = [
    { key: "Overview",  label: t("providerAccount.tabOverview") },
    { key: "Schedule",  label: t("providerAccount.tabSchedule") },
    { key: "Jobs",      label: t("providerAccount.tabJobs") },
    { key: "Reviews",   label: t("providerAccount.tabReviews") },
  ];
  ...
  return (
    <div>
      <ProviderHero
        ...
        email={email} online={online} setOnline={setOnline}
        stats={stats} emailVerified={ownProfile?.emailVerified}
      />

      {bankInfoMissing && (
        <div className="container" style={{ marginTop: 16 }}>
          <div role="alert" style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 18px", borderRadius: 12, background: "var(--amber-50, #fffbeb)", border: "1px solid var(--amber-300, #fcd34d)", color: "var(--amber-900, #78350f)" }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 700, fontSize: 14 }}>{t("providerAccount.bankInfoMissingTitle")}</div>
              <div style={{ fontSize: 13, marginTop: 2, lineHeight: 1.45 }}>{t("providerAccount.bankInfoMissingDesc")}</div>
            </div>
            <Link to="/profile/edit" className="btn btn-primary btn-sm" style={{ flexShrink: 0 }}>
              {t("providerAccount.addNow")}
            </Link>
          </div>
        </div>
      )}
      ...
      <main className="container pro-body">
        <div>
          ...
          {activeTab === "Overview" && (<><ActiveJobCard /><InboundRequests /></>)}
          ...
          {activeTab === "Reviews" && (
            ownProfile?.id ? (
              ...
              <div className="card" style={{ padding: "24px", color: "var(--text-muted)", fontSize: 14 }}>
                {t("providerAccount.loadingReviews")}
              </div>
            )
          )}
        </div>
